    ...
    function (jQuery, createjs) {
        ...
        return {
            start: function () {
                ...
                var display, couleurDefaut, couleurFocus, couleurFocus2, couleurValide, couleurMax,
                    canvas, i,
                    boutonVisuel, boutonTemporel, boutonJournal,
                    spanCopies, spanComparaisons,
                ...
                display = 0; // Mode d'affichage par défaut
                couleurDefaut = '#58c3f0';
                couleurFocus = '#eb9b00';
                couleurFocus2 = '#b0832c';
                couleurValide = '#a8d031';
                couleurMax = '#ff0000';

                // Objets jQuery généraux
                canvas = document.getElementById('visCanvas');
                boutonVisuel = jQuery('#boutonVisuel');
                boutonTemporel = jQuery('#boutonTemporel');
                boutonJournal = jQuery('#boutonJournal');

                // Objets jQuery de la partie visuelle
                spanCopies = jQuery('#visNbCopies');
                spanComparaisons = jQuery('#visNbComparaisons');
                visChoixTri = jQuery('#visChoixTri');
                visChoixAccelere = jQuery('#visChoixAccelere');
                visBoutonStart = jQuery('#visBoutonStart');
                visBoutonStep = jQuery('#visBoutonStep');
                visBoutonRestart = jQuery('#visBoutonRestart');
                visHint0 = jQuery('#visHint0');
                visHint1 = jQuery('#visHint1');
                visHint2 = jQuery('#visHint2');

                // Objets jQuery de la partie temporelle
                tempTailleTab = jQuery('#tempTailleTab');
                ...
                logBoutonReset = jQuery('#logBoutonReset');
                logBoutonCopier = jQuery('#logBoutonCopier');
                ...
                textTab = new createjs.Text('Tableau à trier', 'bold 12px sans-serif', '#000000');
                textTab.x = 225;
                textTab.y = 130;
                textTab.alpha = 0.8;
                ...
                // Le cadre du tableau temporaire pour le tri fusion, identique au premier cadre mais dans la partie inférieure du canvas
                cadreTemp = cadreTab.clone();
                cadreTemp.y = 150;

                // Le cadre de la case temporaire pour les 4 autres types de tri
                cadreTemp2 = new createjs.Shape();
                cadreTemp2.graphics.beginStroke('#000000').
                                    moveTo(211, 250).
                                    lineTo(211, 273).
                                    lineTo(239, 273).
                                    lineTo(239, 250).
                ...
                textTemp = new createjs.Text('Zone temporaire', 'bold 12px sans-serif', '#000000');
                textTemp.x = 225;
                textTemp.y = 280;
                textTemp.alpha = 0.8;
                ...
                function zap(n) {
                    switch (n) {
                    case 0:
                        jQuery("#visuel").show(); // : jQuery("#visuel").hide();
                        jQuery("#temporel").hide();
                        jQuery("#journal").hide();
                        break;
                    case 1:
                        jQuery("#visuel").hide(); // : jQuery("#visuel").hide();
                        jQuery("#temporel").show();
                        jQuery("#journal").hide();
                        break;
                    case 2:
                        jQuery("#visuel").hide(); // : jQuery("#visuel").hide();
                ...
                function newSet(taille) {
                    ...
                    for (i = 0; i < t; i = i + 1) {
                        ...
                        // Javascript ne dispose pas d'une méthode permettant de supprimer directement
                        // un élément d'un tableau, j'utilise donc un petit tour de passe-passe (pas des plus efficaces) :
                        ...
                        // Le tableau des valeurs est deux fois plus grand que la taille demandée pour permettre 
                        // d'effectuer l'algorithme fusion.
                        // Les valeurs de t à 2t+1 sont des zones temporaires initialisées à 0.
                        valeurs[i] = n;
                        valeurs[i + t] = 0;
                    }
                ...
                function Tri(methode, temp, taille) {
                    ...
                    this.copies = 0;
                    this.comparaisons = 0;
                    this.actions = []; // Liste des actions visuelles à effectuer, vide lors de l'initialisation
                    ...
                    this.tick = function () {
                        var time = createjs.Ticker.getTime(),
                            delta = this.accelere ? 100 : 1000; // Période d'appel de doAction, si ce n'est pas un multiple de 10ms la premiére valeur supérieure multiple de 10ms sera retenue.
                        ...
                        if (this.nbActions > 0 && time >= this.derniereAnim + delta && !this.pasAPas) {
                            if (this.doAction() < 2) {
                                this.derniereAnim = time; // On met à jour le temps de dernière exécution d'une animation si besoin.
                            }
                        }
                    };
                    ...
                    // Retourne un entier permettant de préciser si l'animation effectuée doit être succéder d'une pause
                    // en mode pas à pas ou bien si l'animation suivante doit être exécutée immédiatement après en mode 
                    // de lecture normal ou accéléré.
                    this.doAction = function () {
                        ...
                        case 'colorier':
                            ...
                            this.set.
                                affichage[action.n].
                                getChildAt(0).graphics.beginFill(action.c).drawRect(0, 0, 20, 6 * action.n);
                            break;
                        case 'deplacer':
                            // Position de l'élément suivant la valeur de m.
                            xpos = action.m < 32 ? 13 + 27 * (action.m % 16) : 215;
                            ypos = action.m < 16 ? 120 : 270;
                            // Animation de l'élément grace au framework TweenJS. J'utilise la fonction sineOut pour le déplacement, modifiable à souhait.
                            createjs.Tween.get(this.set.affichage[action.n],
                                               {override: true}).to({x: xpos, y: ypos},
                                                                   (this.accelere ? 200 : 700),
                                                                   createjs.Ease.sineOut);
                            // Ici la valeur de retour par défaut est différente des autres éléments
                            // (une pause après chaque déplacement est souhaitable en mode pas à pas),
                            // on force donc un peu
                            // (ne pas mettre la même forme qu'en fin de méthode car 0 || 1 = 1 ...):
                    ...
                    // Met à jour le nombre de copies effectuées lors de l'opération de triage.
                    // Aussi bien en mémoire que sur l'affichage.
                    // Lorsqu'elle est appelée avec l'argument true l'animation qu'elle enregistre est
                    // immédiatement suivi de l'action qui la suit.
                    ...
                    this.incrComparaisons = function (b) {
                        this.comparaisons = this.comparaisons + 1;
                    ...
                    this.selection = function () {
                        var indexMax = 0, i, j;
                        for (i = 0; i < this.taille; i = i + 1) { // i correspond au nombre d'éléments bien posotionnés (en fin de tableau)
                            this.addAction({type: 'text1',
                                            text: 'Phase ' + (i + 1) + ' : trouvons le plus grand élément et déplaçons-le à la position ' + (16 - i)});
                            this.addAction({type: 'text2',
                                            text: 'Pour l\'instant, le premier élément est le maximum.'});
                            indexMax = 0;
                            this.addAction({type: 'colorier',
                                            n: this.set.valeur[0],
                                            c: couleurMax,
                                            r: 1});
                            // On recherche le maximum entre les positions 0 et taille-i
                            for (j = 1; j < this.taille - i; j = j + 1) {
                                this.addAction({type: 'colorier',
                                                n: this.set.valeur[j],
                                                c: couleurFocus});
                                if (this.set.valeur[j] > this.set.valeur[indexMax]) {
                                    this.addAction({type: 'text2',
                                                    text: 'L\'élément ' + (j + 1) + ' est plus grand que l\'élément '
                                                          + (indexMax + 1) + ', il devient donc le max.'});
                                    this.addAction({type: 'colorier',
                                                    n: this.set.valeur[indexMax],
                                                    c: couleurDefaut});
                                    this.addAction({type: 'colorier',
                                                    n: this.set.valeur[j],
                                                    c: couleurMax});
                            ...
                            this.addAction({type: 'text2',
                                            text: 'Permutons l\'élément ' + (16 - i)
                                                  + ' avec le maximum (élément '
                                                  + (indexMax + 1) + ').'});
                            if (indexMax !== this.taille - 1 - i) {
                                this.echanger(this.taille - 1 - i, indexMax);
                            }
                            this.addAction({type: 'colorier',
                                            n: this.set.valeur[this.taille - 1 - i],
                                            c: couleurValide});
                        }
                        return true;
                    };
                    ...
                    this.propagation = function () {
                        var i, j;
                        for (i = 0; i < this.taille; i = i + 1) { // i = nombre d'éléments bien positionnés
                            this.addAction({type: 'text1',
                                            text: 'Phase ' + (i + 1) + ' : le plus grand élément se place en position '
                                                  + (16 - i),
                                            r: 1});
                            // On compare les éléments 2 à 2 et on déace le plus grand vers la fin du tableau
                            for (j = 0; j < this.taille - 1 - i; j = j + 1) {
                                this.addAction({type: 'colorier',
                                                n: this.set.valeur[j],
                                                c: couleurFocus,
                                                r: 2});
                                this.addAction({type: 'colorier',
                                                n: this.set.valeur[j + 1],
                                                c: couleurFocus});
                                if (this.set.valeur[j] > this.set.valeur[j + 1]) {
                                    this.addAction({type: 'text2',
                                                    text: 'L\'élément ' + (j + 1) + ' est plus grand que l\'élément '
                                                          + (j + 2) + ' donc on les permute.',
                                                    r: 1});
                                    this.echanger(j, j + 1);
                                } else {
                                    this.addAction({type: 'text2',
                                                    text: 'L\'élément ' + (j + 1) + ' est plus petit que l\'élément '
                                                          + (j + 2) + ' donc on ne les permute pas.',
                                                    r: 1});
                                }
                                this.incrComparaisons();
                                this.addAction({type: 'colorier',
                    ...
                    this.insertion = function () {
                        ...
                        this.addAction({type: 'colorier',
                                        ...
                                        c: couleurValide});
                        this.addAction({type: 'text1',
                                        text: 'La partie du tableau verte (seulement l\'élément 1 pour l\'instant) est correctement triée.',
                                        r: 1});
                        for (i = 0; i < this.taille - 1; i = i + 1) { // i = dernier élément de la partie triée
                            this.addAction({type: 'colorier',
                                            n: this.set.valeur[i + 1],
                                            c: couleurFocus});
                            this.addAction({type: 'text1',
                                            text: 'Phase ' + (i + 1) + ' : insérons l\'élément '
                                                  + (i + 2) + ' dans la partie triée.'});
                            this.addAction({type: 'text2',
                                            text: 'Plaçons l\'élément ' + (i + 2) + ' dans la zone temporaire.'});
                            this.deplacer(i + 1);
                            j = i;
                            ...
                            while (j >= 0 && this.set.valeur[2 * this.taille] < this.set.valeur[j]) {
                                ...
                                this.addAction({type: 'text2',
                                                text: 'L\'élément ' + (j + 1)
                                                    + ' est plus grand que l\'élément temporaire, on le déplace en position '
                                                    + (j + 2) + '.'});
                                this.deplacer(j);
                                j = j - 1;
                            }
                            ...
                            this.addAction({type: 'text2',
                                            text: j === -1 ? 'L\'élément temporaire est le plus petit, on le copie en position 1.' : 'L\'élément temporaire est plus grand que que l\'élément ' + (j + 1) + ', on a donc trouvé sa position.',
                                            r: 1});
                            this.incrComparaisons();
                    ...
                    this.rapide = function (debut, fin) {
                        var pivot, pos, i, n;
                        this.addAction({type: 'text1',
                                        text: 'On applique le tri rapide aux éléments de '
                                              + (debut + 1) + ' à ' + (fin + 1) + '.'});
                        this.addAction({type: 'text2',
                                        text: 'L\'élément ' + (fin + 1) + ' sert de pivot.'});
                        this.addAction({type: 'colorier',
                                        n: this.set.valeur[fin],
                                        c: couleurMax,
                                        r: 1});
                        // La dernière valeur du sous-tableau servira de valeur pivot.
                        pivot = this.set.valeur[fin];
                        // pos correspond à la position que doit prendre la valeur pivot à la fin de l'algorithme
                        pos = debut;
                        for (i = debut; i < fin; i = i + 1) { // On parcourt les éléments du sous-tableau, on a toujours pos <= i
                            n = this.set.valeur[i];
                            this.addAction({type: 'colorier',
                                            n: n,
                                            c: couleurFocus});
                            if (n < pivot) {
                                // i a une valeur plus petite que la valeur pivot,
                                // on le palce en début de tableau (à la place de pos).
                                this.addAction({type: 'text2',
                                                text: 'L\'élément ' + (i + 1)
                                                      + ' est plus petit que le pivot, on le place dans la première partie du tableau.',
                                                r: 1});
                                ...
                                this.addAction({type: 'text2',
                                                text: 'L\'élément ' + (i + 1)
                                                      + ' est plus grand que le pivot, on le laisse dans la seconde partie du tableau.'});
                            }
                            this.incrComparaisons();
                            this.addAction({type: 'pause'});
                            this.addAction({type: 'colorier',
                        ...
                        this.addAction({type: 'colorier',
                                        n: this.set.valeur[pos],
                                        c: couleurValide});
                        // Appels récursifs pour les deux sous-partie du tableau.
                        if (pos !== debut) { this.rapide(debut, pos - 1); }
                        if (pos !== fin) { this.rapide(pos + 1, fin); }
                        return true;
                    };
                    ...
                    this.fusion = function (debut, fin) {
                        ...
                        if (debut < milieu) { // Si il y a au moins deux éléments dans la première partie, on applique récursivement le tri à cet ensemble
                            ...
                            this.addAction({type: 'text1',
                                            text: 'Appliquons le tri fusion au sous-ensemble ' + (debut + 1)
                                            + ' à ' + (milieu + 1),
                                            r: 2});
                            ...
                            this.addAction({type: 'colorier',
                                            n: this.set.valeur[debut],
                                            c: couleurFocus,
                                            r: 2});
                            this.addAction({type: 'text1',
                                            text: 'L\'élément ' + (debut + 1) + ' est isolé, il est donc trié',
                                            r: 2});
                            this.addAction({type: 'text2',
                                            text: '&nbsp;',
                                            r: 1});
                        ...
                        if (milieu + 1 < fin) { // Idem pour la seconde partie ...
                            ...
                            this.addAction({type: 'text1',
                                            text: 'Appliquons le tri fusion au sous-ensemble '
                                                  + (milieu + 2) + ' à ' + (fin + 1),
                                            r: 2});
                            this.addAction({type: 'text2', text: '&nbsp;', r: 1});
                            ...
                            this.addAction({type: 'colorier',
                                            ...
                                            r: 2});
                            this.addAction({type: 'text1',
                                            text: 'L\'élément ' + (fin + 1) + ' est isolé, il est donc trié',
                                            r: 2});
                            this.addAction({type: 'text2', text: '&nbsp;', r: 1});
                        ...
                        for (k = debut; k < fin + 1; k = k + 1) {
                            this.addAction({type: 'colorier', n: this.set.valeur[k], c: couleurFocus, r: 2});
                        }
                        this.addAction({type: 'text1',
                                        text: 'Fusionnons les sous-ensembles '
                                              + (debut === milieu ? debut + 1 : ((debut + 1) + ' à ' + (milieu + 1)))
                                              + ' et '
                                              + (milieu + 1 === fin ? (fin + 1) : ((milieu + 2) + ' à ' + (fin + 1)))});
                        // On parcourt les emplacements de destination et on y insère à chaque fois le plus petit élément
                        for (k = debut + this.taille; k < fin + this.taille + 1; k = k + 1) {
                            if (i < milieu + 1 && j < fin + 1) { // On vérifie que i et j sont bien où il faut
                                this.addAction({type: 'text2',
                                                text: 'Les plus petits éléments de chaque sous-ensemble sont '
                                                    + (i + 1) + ' et ' + (j + 1),
                                                r: 2});
                                ...
                                if (this.set.valeur[i] < this.set.valeur[j]) {
                                    this.addAction({type: 'text2',
                                                    text: 'L\'élément ' + (i + 1) + ' est plus petit que l\'élément '
                                                          + (j + 1) + ' donc on le place dans la zone temporaire',
                                                    r: 1});
                                    this.deplacer(i, k);
                                    this.addAction({type: 'colorier',
                                                    n: this.set.valeur[k],
                                    ...
                                    this.addAction({type: 'text2',
                                                    text: 'L\'élément ' + (j + 1) + ' est plus petit que l\'élément '
                                                          + (i + 1) + ' donc on le place dans la zone temporaire',
                                                    r: 1});
                                    this.deplacer(j, k);
                                ...
                                if (i > milieu) { // On déplace j
                                    this.addAction({type: 'text2',
                                                    text: 'Le premier sous-ensemble est vide, on recopie le plus petit élément du second (' + (j + 1) + ')',
                                                    r: 1});
                                    this.addAction({type: 'colorier',
                                                    n: this.set.valeur[j],
                                                    c: couleurFocus2});
                                    ...
                                    this.addAction({type: 'text2',
                                                    text: 'Le second sous-ensemble est vide, on recopie le plus petit élément du premier (' + (i + 1) + ')',
                                                    r: 1});
                        ...
                        for (k = debut + this.taille; k < fin + this.taille + 1; k = k + 1) {
                            ...
                            this.addAction({type: 'colorier',
                                            n: this.set.valeur[k - this.taille],
                                            c: (debut === 0 && fin === this.taille - 1 ? couleurValide : couleurDefaut),
                                            r: 2});
                        }
                        return true;
                    };
                    ...
                    this.trier = function () {
                        var string, b; // b contient le résultat du tri.
                ...
                visBoutonStep.on('click', function () {
                    ...
                    actionOK = tri.doAction();
                    while (actionOK === 1) { actionOK = tri.doAction(); } // On effectue les actions jusqu'à rencontrer une pause.
                });
                ...
                tempBoutonStart.on('click', function () {
                    ...
                    var t, n, methode, tempsCalcul, tempsTotal, copies, comparaisons, nbTries, i, dt, string;
                    t = parseInt(tempTailleTab.val(), 0);
                    n = parseInt(tempNbTab.val(), 0);
                    methode = tempChoixTri.val();
                    // On initialise les temps de calcul et les variables
                    tempsCalcul = 0;
                    tempsTotal = new Date().getTime();
                    copies = 0;
                    comparaisons = 0;
                    nbTries = 0;
                    ...
                    for (i = 0; i < n; i = i + 1) {
                        ...
                        dt = new Date().getTime(); // Début du calcul
                        if (tri.trier()) { nbTries = nbTries + 1; }
                ...
                tri = new Tri(5); // Par défaut : tri par sélection
                tri.init();
                createjs.Ticker.addListener(tri);
            }
        };
